...
final class AppDelegateActionsTests: XCTestCase {
    ...
    // MARK: - Hidden Icon Warning State Tests

    func testHiddenIconWarningState_waitsForStableHiddenInterval() {
        var state = HiddenIconWarningState()
        let start = Date(timeIntervalSinceReferenceDate: 0)

        XCTAssertFalse(state.shouldWarn(hiddenNow: true, suppressed: false, now: start, warningDelay: 2))
        XCTAssertFalse(state.shouldWarn(
            hiddenNow: true,
            suppressed: false,
            now: start.addingTimeInterval(1.9),
            warningDelay: 2
        ))
        XCTAssertTrue(state.shouldWarn(
            hiddenNow: true,
            suppressed: false,
    ...
    func testHiddenIconWarningState_warnsOnceUntilIconIsVisibleAgain() {
        var state = HiddenIconWarningState()
        let start = Date(timeIntervalSinceReferenceDate: 0)
    ...
    func testHiddenIconWarningState_rearmsAfterVisibleSample() {
        var state = HiddenIconWarningState()
        let start = Date(timeIntervalSinceReferenceDate: 0)
    ...
    func testHiddenIconWarningState_respectsSuppression() {
        var state = HiddenIconWarningState()
        let start = Date(timeIntervalSinceReferenceDate: 0)

        XCTAssertFalse(state.shouldWarn(hiddenNow: true, suppressed: true, now: start, warningDelay: 2))
        XCTAssertFalse(state.shouldWarn(
            hiddenNow: true,
            suppressed: true,
            now: start.addingTimeInterval(20),
