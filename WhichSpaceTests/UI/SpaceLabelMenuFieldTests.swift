...
struct SpaceLabelMenuFieldTests {
    ...
    @Test("status menu places the label field between the version header and Settings")
    func menuPlacesFieldUnderHeader() {
        ...
        #expect(menu.items[2].view === field)
        #expect(menu.items[3].isSeparatorItem)
        #expect(menu.items[4].title == Localization.menuSettingsWindow)
