...
final class StatusBarRenderer {
    ...
    func generatePreviewIcon(
        ...
        } else if showAllSpaces, !appState.allSpaceLabels.isEmpty {
            result = generateCombinedIcon(darkMode: isDark)
        } else {
            result = generateSingleIcon(
                for: appState.currentSpace, label: appState.currentSpaceLabel, darkMode: isDark
            )
        }
    ...
    func statusBarLayout() -> StatusBarLayout {
        ...
        if showAllSpaces {
            ...
            for (drawIndex, spaceInfo) in spacesToShow.enumerated() {
                ...
                let target = isFullscreen ? nil : shortcutNum
                let localRegularIndex = appState.allDisplaysSpaceInfo
                    .first { $0.displayID == appState.currentDisplayID }?
                ...
                let spaceID = appState.allSpaceIDs[spaceInfo.index]
                slots.append(StatusBarIconSlot(
    ...
    private func buildIconCacheKey() -> IconCacheKey {
        ...
        return IconCacheKey(
            ...
            allSpaceLabels: appState.allSpaceLabels,
    ...
    private func generateStatusBarIcon(isDark: Bool) -> NSImage {
        ...
        if showAllSpaces, !appState.allSpaceLabels.isEmpty {
    ...
    private func generateSingleIcon(for space: Int, label: String, darkMode: Bool) -> NSImage {
        ...
        var colors = SpacePreferences.colors(
            forSpace: space, display: appState.currentDisplayID, store: store
        )
        var style = SpacePreferences.iconStyle(
            forSpace: space, display: appState.currentDisplayID, store: store
        ) ?? .square
        let font = SpacePreferences.font(
            forSpace: space, display: appState.currentDisplayID, store: store
        )?.font
        ...
        let symbol = (isCurrentSpace && previewClearSymbol)
            ? nil
            : SpacePreferences.symbol(forSpace: space, display: appState.currentDisplayID, store: store)

        if let symbol {
            // Use per-space skin tone, defaulting to yellow  (rather than the global emoji picker preference)
            let skinTone = SpacePreferences
                .skinTone(forSpace: space, display: appState.currentDisplayID, store: store) ?? .default
            return SpaceIconGenerator.generateSymbolIcon(
                symbolName: symbol,
                darkMode: darkMode,
                customColors: colors,
    ...
    private func spacesToShowForCurrentDisplay() -> [(index: Int, label: String)] {
        ...
        if needsFiltering {
            ...
            let filtered = appState.allSpaceLabels.enumerated().filter { index, label in
                let spaceID = appState.allSpaceIDs[index]
        ...
        return appState.allSpaceLabels.enumerated().map { (index: $0.offset, label: $0.element) }
    }

    private func spacesToShowAcrossDisplays() -> [[CrossDisplaySpace]] {
        // Collect all space IDs for window detection
        let allSpaceIDsAcrossDisplays = appState.allDisplaysSpaceInfo.flatMap(\.spaceIDs)
        let nonEmptySpaceIDs: Set<Int>
        if store.hideEmptySpaces {
        ...
        for displayInfo in appState.allDisplaysSpaceInfo {
            ...
            for (arrayIndex, label) in displayInfo.labels.enumerated() {
                ...
                let isActive = spaceID == appState.currentSpaceID
